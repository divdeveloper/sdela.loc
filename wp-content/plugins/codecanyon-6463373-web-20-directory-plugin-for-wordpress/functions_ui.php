...
function w2dc_orderLinks($base_url, $defaults = array(), $return = false, $shortcode_hash = null) {
	...
	if ($current_category = w2dc_is_category()) {
...
function w2dc_renderAllCategories($parent = 0, $depth = 2, $columns = 2, $count = false, $max_subcategories = 0, $allowed_levels = array(), $exact_categories = array(), $hide_empty = false) {
	if ($depth > 2)
		$depth = 2;
	...
	if ($terms) {
		foreach ($terms AS $key=>$term) {
			...
			foreach ($term_children AS $term_id) {
				if ($term_child = get_term($term_id, W2DC_CATEGORIES_TAX)) {
		...
		if ($terms) {
			...
			foreach ($terms AS $key=>$term) {
				...
				echo _w2dc_renderAllCategories($term->term_id, $depth, 1, $count, $max_subcategories, $allowed_categories, $exact_categories, $hide_empty);
				echo '</div>';
...
function _w2dc_renderAllCategories($parent = 0, $depth = 2, $level = 0, $count = false, $max_subcategories = 0, $allowed_categories = array(), $exact_categories = array(), $hide_empty = false) {
	if ($count)
		// there is a wp bug with pad_counts in get_terms function - so we use this construction
...
function w2dc_renderAllLocations($parent = 0, $depth = 2, $columns = 2, $count = false, $max_sublocations = 0, $exact_locations = array(), $hide_empty = false) {
	if ($depth > 2)
		$depth = 2;
	if ($depth == 0 || !is_numeric($depth))
		$depth = 1;
	...
	if ($terms) {
		foreach ($terms AS $key=>$term) {
			...
			if (is_array($term_children))
			foreach ($term_children AS $term_id) {
				if ($term_child = get_term($term_id, W2DC_LOCATIONS_TAX)) {
					$term_children_slugs[] = $term_child->slug;
				}
			}
		...
		if ($terms) {
			...
			foreach ($terms AS $key=>$term) {
				...
				echo _w2dc_renderAllLocations($term->term_id, $depth, 1, $count, $max_sublocations, $exact_locations, $hide_empty);
				echo '</div>';
...
function _w2dc_renderAllLocations($parent = 0, $depth = 2, $level = 0, $count = false, $max_sublocations = 0, $exact_locations = array(), $hide_empty = false) {
	if ($count)
		// there is a wp bug with pad_counts in get_terms function - so we use this construction
		$terms = wp_list_filter(
				get_categories(array(
		...
		if (get_class($query) == 'WP_Query') {
			...
			if ($total_pages > 1){
				...
				if (!$show_more_button) {
					...
					echo '<button class="w2dc-btn w2dc-btn-primary w2dc-btn-lg w2dc-btn-block w2dc-show-more-button" data-controller-hash="' . $hash . '">' . sprintf(__('Show more %s', 'W2DC'), $directory->plural) . '</button>';
				}
			}
		}
	}
