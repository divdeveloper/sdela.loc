...
class w2dc_plugin {
	...
	public function enqueue_scripts_styles($load_scripts_styles = false) {
		...
		if ((($this->frontend_controllers || $load_scripts_styles) && !$w2dc_enqueued) || get_option('w2dc_force_include_js_css')) {
			...
			wp_register_style('w2dc_media_styles_q_confirm', W2DC_RESOURCES_URL . 'jquery-confirm/css/jquery-confirm.css');
			wp_register_script('w2dc_media_scripts_q_confirm', W2DC_RESOURCES_URL . 'jquery-confirm/js/jquery-confirm.min.js', array('jquery'), false, true);
			...
			if ($this->getShortcodeProperty('webdirectory', 'is_single') || $this->getShortcodeProperty(W2DC_LISTING_SHORTCODE, 'is_single') || $this->getShortcodeProperty('webdirectory-listing', 'is_single')) {
				...
				wp_enqueue_style('w2dc_media_styles_q_confirm');
				wp_enqueue_script('w2dc_media_scripts_q_confirm');
			}
...
';
		...
		echo 'var w2dc_js_objects = ' . json_encode(
				array(
						...
						'in_favourites_msg' => __('Добавить в ИЗБРАННОЕ', 'W2DC'),
						'not_in_favourites_msg' => __('Удалить из ИЗБРАННОГО', 'W2DC'),
						'in_interests_msg' => __('Мне это интересно', 'W2DC'),
						'not_in_interests_msg' => __('Удалить из ИНТЕРЕС', 'W2DC'),
						'close_btn_msg' => __('закрыть', 'W2DC'),
