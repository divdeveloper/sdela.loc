...
class w2dc_listing {
	...
	public function display($is_single = false, $return = false) {
		...
		return w2dc_renderTemplate($template, array('listing' => $this, 'is_single' => $is_single), $return);
	}
	...
	public function changeLevel($new_level_id, $invoke_hooks = true) {
		...
		if ((isset($w2dc_instance->levels->levels_array[$new_level_id]) && !$this->level->upgrade_meta[$new_level_id]['disabled']) || (current_user_can('editor') || current_user_can('administrator'))) {
			$listings = array($this);
