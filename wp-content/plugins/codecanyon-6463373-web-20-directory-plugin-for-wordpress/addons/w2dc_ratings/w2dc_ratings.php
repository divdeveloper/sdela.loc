	...
	*/
	...
	public function enqueue_scripts_styles($load_scripts_styles = false) {
		...
		if ((is_admin() || $w2dc_instance->frontend_controllers || $load_scripts_styles) && !$w2dc_ratings_enqueued) {
			wp_register_script('w2dc_rater', W2DC_RATINGS_RESOURCES_URL . 'js/jquery.rater.js', array('jquery'), false, true);
			wp_register_style('w2dc_rater', W2DC_RATINGS_RESOURCES_URL . 'css/rater.css');

			if (function_exists('is_rtl') && is_rtl()) {
				wp_register_script('w2dc_rater_rtl', W2DC_RATINGS_RESOURCES_URL . 'js/jquery.rater-rtl.js', array('jquery'), false, true);
				wp_register_style('w2dc_rater_rtl', W2DC_RATINGS_RESOURCES_URL . 'css/rater-rtl.css');
			}

			wp_enqueue_script('w2dc_rater');
			wp_enqueue_style('w2dc_rater');

			wp_enqueue_script('w2dc_rater_rtl');
			wp_enqueue_style('w2dc_rater_rtl');
