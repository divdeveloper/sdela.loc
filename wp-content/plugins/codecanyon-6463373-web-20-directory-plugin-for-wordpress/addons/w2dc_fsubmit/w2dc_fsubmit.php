...
class w2dc_fsubmit_plugin {
	...
	public function add_submit_button($listing, $frontend_controller) {
		global $w2dc_instance;

		if (get_option('w2dc_fsubmit_button') && !empty($w2dc_instance->submit_pages_all)) {
			$page_id = get_the_ID();
			...
			if ($w2dc_instance->directories->isMultiDirectory() && (in_array($page_id, $submit_pages) || $page_id == $w2dc_instance->dashboard_page_id)) {
				foreach ($w2dc_instance->directories->directories_array AS $directory) {
					$href = apply_filters('w2dc_submit_button_href', w2dc_submitUrl(array('directory' => $directory->id)), $directory);

					echo '<a class="w2dc-submit-listing-link w2dc-btn w2dc-btn-primary" href="' . $href . '" rel="nofollow"><span class="w2dc-glyphicon w2dc-glyphicon-plus"></span> ' . sprintf(__('Submit new %s', 'W2DC'), $directory->single) . '</a> ';
				}
			} else {
				if ($frontend_controller && isset($frontend_controller->args['directory'])) {
					$directory = $w2dc_instance->directories->getDirectoryById($frontend_controller->args['directory']);
				} else {
	...
	public function remove_admin_bar() {
		if (get_option('w2dc_hide_admin_bar') && !current_user_can('administrator') && !current_user_can('editor') && !is_admin()) {
	...
	public function enqueue_scripts_styles($load_scripts_styles = false) {
		...
		if (($w2dc_instance->frontend_controllers || $load_scripts_styles) && !$w2dc_fsubmit_enqueued) {
			...
			if (function_exists('is_rtl') && is_rtl()) {
				wp_register_style('w2dc_fsubmit_rtl', W2DC_FSUBMIT_RESOURCES_URL . 'css/submitlisting-rtl.css');
			}

			wp_enqueue_style('w2dc_fsubmit');
			wp_enqueue_style('w2dc_fsubmit_rtl');
