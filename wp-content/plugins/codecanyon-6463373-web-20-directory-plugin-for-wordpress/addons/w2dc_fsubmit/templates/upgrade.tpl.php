...
<form action="<?php echo w2dc_dashboardUrl(array('w2dc_action' => 'upgrade_listing', 'listing_id' => $w2dc_instance->current_listing->post->ID, 'upgrade_action' => 'upgrade', 'referer' => urlencode($frontend_controller->referer))); ?>" method="POST">
	...
	<?php if ($w2dc_instance->current_listing->level->id != $level->id && (!isset($w2dc_instance->current_listing->level->upgrade_meta[$level->id]) || !$w2dc_instance->current_listing->level->upgrade_meta[$level->id]['disabled']|| (current_user_can('editor') || current_user_can('administrator')))): ?>
	<p>
