...
class w2dc_listings_package_product {
	...
	public function get_all_packages() {
		...
		$packages = apply_filters('w2dc_wc_packages', $packages);
	...
	public function set_listings_numbers($status, $order_id) {
		if ($status == 'completed') {
			...
			foreach ($items AS $item_id=>$item) {
				if (is_a($item, 'WC_Order_Item_Product') && ($product = wc_get_product($item->get_product_id())) && $product->product_type == 'listings_package') {
